    ...
    if not user_cart_id:
        if cart_id:
            if database.check_cart_id(cart_id):
                ...
                current_cart = database.update_cart(cart_id, product, quantity, user_id)
            else:
                return {
                    "statusCode": 404,
                    "headers": utility.get_headers(cart_id=cart_id),
        ...
        if cart_id:
            if cart_id == user_cart_id:
                ...
                current_cart = database.update_cart(user_cart_id, product, quantity, user_id)
            elif database.check_cart_id(cart_id):
                database.renew_cart_expiry(cart_id, user_id)
                database.update_cart(cart_id, product, quantity, user_id)
